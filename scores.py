...
class GridScorer(object):
    ...
    def calculate_ratemap(self, xs, ys, rate_map, statistic='mean'):
    ...
    def calculate_sac(self, seq1):
        ...
        # std_seq2 = np.power(np.divide(sum_seq2_sq, n_bins) - np.divide(sum_seq2**2, n_bins_sq),0.5)
        covar = np.subtract(
                np.divide(seq1_x_seq2, n_bins),
                np.divide(np.multiply(sum_seq1, sum_seq2), n_bins_sq))
        # x_coef = np.divide(covar, np.multiply(std_seq1, std_seq2))
        x_coef = np.divide(covar, np.multiply(std_seq1, std_seq2), out=np.zeros_like(covar), where=covar!=0)
